...
export default defineEventHandler(async event => {
	...
	console.log(message);
	console.log(messageListWithSystemMessage);

	const getTokenCount = () =>
		TokenCounter[message.data.model].encode(
			messageListWithSystemMessage.map(({ content }) => content).join('\n\n')
		).length;
	...
	if (getTokenCount() > MaxTokensPerModel[message.data.model]) {
		...
		while (
			messageListWithSystemMessage.length > 1 &&
			lastTokenCount > MaxTokensPerModel[message.data.model]
		) {
		...
		if (messageListWithSystemMessage.length === 1) {
			...
			return {
				error: {
					...
					message: `The token count for the messages is too high. The maximum is ${
						MaxTokensPerModel[message.data.model]
					}, but the current count is ${lastTokenCount}.`,
				},
			};
		}
	}
